...
# These patches are from the Debian package.
termux_step_patch_package() {
	cd $TERMUX_PKG_SRCDIR
	patch -p1 -i ${TERMUX_PKG_BUILDER_DIR}/fix_manpage_lintian_warnings.patch
	patch -p1 -i ${TERMUX_PKG_BUILDER_DIR}/fix_memleak_in_my_wnck_get_viewport_start.patch
	patch -p1 -i ${TERMUX_PKG_BUILDER_DIR}/fix_using_deprecated_wnck_functions.patch
	patch -p1 -i ${TERMUX_PKG_BUILDER_DIR}/remove_unavailable_options_from_manpage.patch	
}
